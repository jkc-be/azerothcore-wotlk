...
const chrome = spawn('/usr/bin/google-chrome', ['--headless', '--no-sandbox', '--disable-dev-shm-usage',
...
try {
  ...
  const send = (method, params = {}) => new Promise((resolve, reject) => {
    ...
    pending.set(id, data => { clearTimeout(timeout); if (data.error) reject(data.error); else resolve(data.result); });
    socket.send(JSON.stringify({ id, method, params }));
  });
  const evaluate = async expression => (await send('Runtime.evaluate', { expression, returnByValue: true,
  ...
  await evaluate('document.querySelector("#token").value = "fixture-token"; document.querySelector("#connect").requestSubmit()');
  ...
  console.log('PASS: real Chromium UI, fixture SSE, bot selection, pause acknowledgement and speed controls; no JS exceptions.');
} finally {
