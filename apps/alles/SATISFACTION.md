...
observed attempt. Preferences remain authored, rather than changing from a model-generated explanation.
...
place activities, at most two detours around a visible threat are compared using the same satisfaction model.
...
alternative. Unsupported cross-map routes cannot be executed. Quest destinations remain the existing tactical
executor's responsibility; until it has an approach position their travel estimate is an uncertain prior.
