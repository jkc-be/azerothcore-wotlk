...
{
    if (conversations.size() >= 32 || conversationResults.size() >= 64 || id.empty() || id.size() > 64 ||
        boost::json::serialize(context).size() > 10000)
        return false;
    conversations.push_back({std::move(id), std::move(context), realMs, realMs + 45000});
    return true;
}
...
boost::json::object Service::Status() const
{
    return {{"mode", "provider"},
            ...
            {"conversationQueued", conversations.size()},
...
void Service::Update(uint64_t gameMs, uint64_t realMs)
{
    ...
    std::erase_if(conversations,
                  [&](Conversation const& item)
                  {
                      if (realMs < item.expires && !(item.cancelled && item.state == "queued"))
                          return false;
                      if (!item.cancelled && conversationResults.size() < 64)
                          conversationResults.push_back({item.id, "timeout", {}});
                      ++conversationFailed;
                      return true;
                  });
                                    ...
                                    uint64_t realMs)
{
    ...
    lastWorkerMs = realMs;
    if (op == "submit_conversation")
    {
        Fields(args, {"jobToken", "permitId", "response", "outcome", "promptTokens", "completionTokens", "latencyMs"});
        auto found = std::find_if(conversations.begin(), conversations.end(),
                                  [&](auto const& item) { return item.id == String(args, "jobToken", 64); });
        if (found == conversations.end() || found->worker != worker || found->state != "running" ||
        ...
        if (outcome == "success")
        {
            ...
            if (!IsBoundedText(text, 255) || (reply && text.empty()) ||
                (!reply && (!text.empty() || action != "none")) || text.find('|') != std::string::npos ||
                std::any_of(text.begin(), text.end(), [](unsigned char c) { return c < 32 || c == 127; }) ||
                (action != "none" && action != "follow" && action != "stop" && action != "assist" && action != "wave"))
                throw std::invalid_argument("invalid conversation response");
        }
        else if (outcome != "failed")
            throw std::invalid_argument("invalid outcome");
        LOG_INFO("module.alles", "Ollama conversation {} outcome={} prompt={} completion={} latencyMs={}", found->id,
                 outcome, Number(args, "promptTokens"), Number(args, "completionTokens"), Number(args, "latencyMs"));
        if (!found->cancelled && conversationResults.size() < 64)
            conversationResults.push_back({found->id, outcome, std::move(response)});
        if (outcome == "success")
        {
    ...
    if (op == "next_jobs")
    {
        ...
        for (auto& item : conversations)
            if (!item.cancelled && item.worker == worker && item.state == "granted")
            {
                ...
                return {{"conversation", boost::json::object{{"jobToken", item.id},
                                                             {"permitId", item.permit},
                                                             {"remainingMs", item.expires - realMs},
        ...
        {
            auto next =
                std::find_if(conversations.begin(), conversations.end(), [&](auto const& item)
                             { return !item.cancelled && item.state == "queued" && realMs + 25000 <= item.expires; });
            if (next != conversations.end())
            {
                Charge(gameMs);
                next->worker = worker;
                next->permit = "chat-" + std::to_string(gameMs) + "-" + std::to_string(charged);
                next->state = "pending";
                transport.Reserve(next->permit,
                                  boost::json::serialize(boost::json::object{{"profile", settings.profile},
                                                                             {"permit", next->permit},
                                                                             {"job", next->id},
                                                                             {"model", settings.model},
